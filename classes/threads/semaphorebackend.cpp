...
S::Array<S::Threads::SemaphoreBackend *(*)(S::Void *)>	*S::Threads::SemaphoreBackend::backend_creators = NIL;

S::Int S::Threads::SemaphoreBackend::AddBackend(SemaphoreBackend *(*backend)(Void *))
{
	if (backend == NIL) return Failure;

	if (backend_creators == NIL) backend_creators = new Array<SemaphoreBackend *(*)(Void *)>;
