...
#include <smooth/graphics/xlib/surfacexlib.h>
...
S::Int S::GUI::SurfaceXLib::Line(Point pos1, Point pos2, Int color)
{
	if (window == NIL) return Success;
...
S::Int S::GUI::SurfaceXLib::Box(Rect rect, Int color, Int style)
{
	if (window == NIL) return Success;
...
S::Int S::GUI::SurfaceXLib::SetText(String string, Rect rect, Font font, Bool shadow)
{
	if (window == NIL)	return Success;
...
S::Int S::GUI::SurfaceXLib::BlitFromBitmap(const Bitmap &oBitmap, Rect srcRect, Rect destRect)
{
	if (window == NIL) return Success;
...
S::Int S::GUI::SurfaceXLib::BlitToBitmap(Rect srcRect, const Bitmap &oBitmap, Rect destRect)
{
	if (window == NIL) return Success;
