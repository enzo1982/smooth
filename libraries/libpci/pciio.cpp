...
SMOOTHString	 cfNames[10];
SMOOTHString	 cpNames[5];
...
void PCIIO::SetDescription(SMOOTHString d)
{
	description = d;
}

void PCIIO::SelectImage(SMOOTHString name)
{
	imagename = name;
}
...
void PCIIO::SetImageName(SMOOTHString name)
{
	imagename = name;
}
...
PCIOut CreatePCI(SMOOTHString filename)
{
	return new OutStream(STREAM_FILE, filename, OS_OVERWRITE);
}

PCIOut OpenPCIForOutput(SMOOTHString filename)
{
	return new OutStream(STREAM_FILE, filename, OS_APPEND);
}

PCIIn OpenPCIForInput(SMOOTHString filename)
{
...
bool ReadPCIFTAG(PCIIn in, PCIIO &ior)
{
	...
	do
	{
		...
		if (SMOOTHString(tag).Compare("PCIF") == 0)
		{
			delete [] tag;
...
bool ReadIMAGTAG(PCIIn in, PCIIO &ior)
{
	...
	do
	{
		...
		if (SMOOTHString(tag).Compare("IMAG") == 0)
		{
			delete [] tag;
...
bool ReadRESOTAG(PCIIn in, PCIIO &ior)
{
	...
	do
	{
		...
		if (SMOOTHString(tag).Compare("RESO") == 0)
		{
			delete [] tag;
...
bool ReadFORMTAG(PCIIn in, PCIIO &ior)
{
	...
	do
	{
		...
		if (SMOOTHString(tag).Compare("FORM") == 0)
		{
			delete [] tag;
...
bool ReadDESCTAG(PCIIn in, PCIIO &ior)
{
	...
	do
	{
		...
		if (SMOOTHString(tag).Compare("DESC") == 0)
		{
			delete [] tag;
...
bool ReadNAMETAG(PCIIn in, PCIIO &ior)
{
	...
	do
	{
		...
		if (SMOOTHString(tag).Compare("NAME") == 0)
		{
			delete [] tag;
...
bool ReadDATATAG(PCIIn in, PCIIO &ior)
{
	...
	do
	{
		...
		if (SMOOTHString(tag).Compare("DATA") == 0)
		{
			delete [] tag;
...
bool FindImageID(PCIIn in, PCIIO &ior)
{
	...
	do
	{
		...
		if (SMOOTHString(tag).Compare("NAME") == 0)
		{
			...
			if (SMOOTHString(iname).Compare(ior.imagename) == 0)
			{
				delete [] iname;
